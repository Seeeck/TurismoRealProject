...
class ListaDepartamentosView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        lista = Departamento.objects.filter(
            id_zona__comuna__icontains=comuna, estado_departamento=True)

        return lista

    def get_context_data(self, **kwargs):
        context = super(ListaDepartamentosView,
...
class ReservarDepartamentoView(SuccessMessageMixin, LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(ReservarDepartamentoView,
                        self).get_context_data(**kwargs)
        cliente = Cliente.objects.get(user_cliente=self.request.user)
        ...
        sv_transporte = Sv_Transporte.objects.get(
            id_sv_transporte=departamento.id_sv_transporte.id_sv_transporte)
    ...
    def form_valid(self, form):
        ...
        checkin = CheckIn.objects.create(
            fecha_checkin=self.request.POST.get('check_in'))
        checkout = CheckOut.objects.create(
            fecha_checkout=self.request.POST.get('check_out'))
        ...
        for key, value in self.request.POST.items():
            if (str(key).__contains__('nombre') or str(key).__contains__('apellido')) and str(value) != '':
                if(str(key).__contains__('nombre')):
                    ...
                    p = PersonaExtra.objects.create(
                        nombre=nombre_persona, id_reserva=reserva)
        ...
        if(self.request.POST.get('transporteCheck') == 'true'):
            ...
            precio_transporte = (
                departamento.id_sv_transporte.valor_transporte)*cantidad_personas
            ...
            transporte = Transporte.objects.create(fecha_ida=self.request.POST.get('check_in'),
                                                   fecha_vuelta=self.request.POST.get(
                                                       'check_out'),
...
def EditarReservaView(request, id_reserva):
    ...
    if(request.method == 'POST'):
        ...
        reserva = Reserva.objects.get(
            id_cliente=cliente, id_reserva=id_reserva)
        ...
        cantidad_personas = 0
        id_transporte = reserva.id_transporte
        is_tour = False
        ...
        cantidad_personas = PersonaExtra.objects.filter(
        id_reserva=id_reserva).count()+1

        if(request.POST.get('tourCheck') == 'true'):
            is_tour = True
            reserva.is_tour = is_tour
        ...
        if(request.POST.get('transporteCheck') == 'true'):
            ...
            transporte = Transporte.objects.create(fecha_ida=reserva.id_check_in.fecha_checkin,
                                                   ...
                                                   direccion_inicio=request.POST.get('direccionInicioTransporte'), id_sv_transporte=sv_transporte)

            transporte.save()
            id_transporte = transporte.pk
